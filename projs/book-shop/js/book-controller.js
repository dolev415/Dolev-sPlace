...
function renderBooks() {
    ...
    if (books.length === 0) {
        ...
        <td>${book.rating}</td>
        <td class="book${book.id}">$${book.price}</td>
        <td> <button id="${book.id}" class="read-btn" onclick="onReadBook(this.id)">Read</button> </td>
        <td> <button id="${book.id}" class="update-btn" onclick="onUpdateBook(this)">Update</button> </td>
        <td> <button id="${book.id}" class="delete-btn" onclick="onRemoveBook(this.id)">Delete</button> </td>
        </tr>`
        })
...
function onUpdateBook(updateBtn) {
    ...
    if (btnTxt === 'Update') {
        elPriceTd.innerHTML = `<input name="price-update" type="number" placeholder="Update">`
